...
masdir = os.environ['DBOX'] + 'Data\\MAS\\'
r_out = 30*u.solRad
...
#https://www.predsci.com/data/runs/cr2255-high/hmi_mast_mas_std_0201/corona/br002.hdf

def get_MAS_output(cr=np.NaN, observatory='', runtype='', runnumber='', masres=''):
    """
    ...
    # https://www.predsci.com/data/runs/cr1625-medium/kpo_mas_mas_std_0101/corona/br002.hdf
    # https://www.predsci.com/data/runs/cr2062-medium/mdi_mas_mas_std_0101/corona/br002.hdf
    mas_url_front = 'https://www.predsci.com/data/runs/cr'
    mas_url_end = '002.hdf'
        ...
        for masob in observatories_order:
            for masrun in runtype_order:
                for masnum in runnumber_order:
                    urlbase = (mas_url_front + str(int(cr)) + '-' + 
                               res + '/' + masob + '_' +
                               masrun + '_mas_std_' + masnum + '/corona/')
                    url = urlbase + 'br' + mas_url_end
    ...
    urllib.request.urlretrieve(urlbase + 'br' + mas_url_end,
                               os.path.join(brfilename))
    urllib.request.urlretrieve(urlbase + 'vr' + mas_url_end,
                               os.path.join(vrfilename))
    
        
    return 1

def read_MAS(filepath):
    """
...
for obs in observatories: 
    ...
    for CR in range(CRstart,CRstop):
        brfilepath = masdir + obs + '\\CR' + str(CR) + '\\br002.hdf'
        ...
        if (os.path.exists(brfilepath) == True):
            ...
            #load the Br data
            Br, BXa, BXm, BXr = read_MAS(brfilepath) 
            Br = Br *  2.2e5
            

            
            #take the slice at r = 215 rS           
            id_r_b = np.argmin(abs(BXr - r_out))
...
plt.plot(df['datetime'],
         ...
         label = 'HelioMAS; ' + str(r_out.value) +' r_S')
