...
#find teh CR range
CRstart = int(np.ceil(htime.mjd2crnum(df['mjd'][0]))) # 1625
CRstop =  int(np.floor(htime.mjd2crnum(df['mjd'][len(df)-1]))) - 2 # 1625
nCRs = CRstop-CRstart +1
...
flux_obs = np.ones((nCRs))
flux_gaps =  np.ones((nCRs))
fluxSS_obs = np.ones((nCRs))
fluxnonSS_obs = np.ones((nCRs))
fluxSS_gaps = np.ones((nCRs))
fluxnonSS_gaps = np.ones((nCRs))

flux_obs_sw = np.ones((nCRs))
flux_gaps_sw =  np.ones((nCRs))
fluxSS_obs_sw = np.ones((nCRs))
fluxnonSS_obs_sw = np.ones((nCRs))
fluxSS_gaps_sw = np.ones((nCRs))
fluxnonSS_gaps_sw = np.ones((nCRs))

flux_obs_icme = np.ones((nCRs))
flux_gaps_icme =  np.ones((nCRs))
fluxSS_obs_icme = np.ones((nCRs))
fluxnonSS_obs_icme = np.ones((nCRs))
fluxSS_gaps_icme = np.ones((nCRs))
fluxnonSS_gaps_icme = np.ones((nCRs))

flux_obs_sh = np.ones((nCRs))
flux_gaps_sh =  np.ones((nCRs))
fluxSS_obs_sh = np.ones((nCRs))
fluxnonSS_obs_sh = np.ones((nCRs))
fluxSS_gaps_sh = np.ones((nCRs))
fluxnonSS_gaps_sh = np.ones((nCRs))

N_tot = np.ones((nCRs))
N_tot_icme = np.ones((nCRs))
N_tot_sw = np.ones((nCRs))
N_tot_sh = np.ones((nCRs))

N_gaps = np.ones((nCRs))
N_gaps_sw = np.ones((nCRs))
N_gaps_icme = np.ones((nCRs))
N_gaps_sh = np.ones((nCRs))

i = 0


df_cr = pd.DataFrame()
df_cr['CR'] = np.arange(CRstart,CRstop+1,1)
df_cr['mjd'] = htime.crnum2mjd(df_cr['CR'] + 0.5)
df_cr['datetime'] = htime.mjd2datetime(df_cr['mjd'].to_numpy())

df_cr['N_tot'] = np.ones((nCRs))*0
df_cr['N_gaps'] = np.ones((nCRs))*0
df_cr['flux_obs'] = np.ones((nCRs))*0
df_cr['flux_gaps'] =  np.ones((nCRs))*0
df_cr['fluxSS_obs'] = np.ones((nCRs))*0
df_cr['fluxnonSS_obs'] = np.ones((nCRs))*0
df_cr['fluxSS_gaps'] = np.ones((nCRs))*0
df_cr['fluxnonSS_gaps'] = np.ones((nCRs))*0

df_cr['N_tot_am'] = np.ones((nCRs))*0
df_cr['N_gaps_am'] = np.ones((nCRs))*0
df_cr['flux_obs_am'] = np.ones((nCRs))*0
df_cr['flux_gaps_am'] =  np.ones((nCRs))*0
df_cr['fluxSS_obs_am'] = np.ones((nCRs))*0
df_cr['fluxnonSS_obs_am'] = np.ones((nCRs))*0
df_cr['fluxSS_gaps_am'] = np.ones((nCRs))*0
df_cr['fluxnonSS_gaps_am'] = np.ones((nCRs))*0

df_cr['N_tot_sh'] = np.ones((nCRs))*0
df_cr['N_gaps_sh'] = np.ones((nCRs))*0
df_cr['flux_obs_sh'] = np.ones((nCRs))*0
df_cr['flux_gaps_sh'] =  np.ones((nCRs))*0
df_cr['fluxSS_obs_sh'] = np.ones((nCRs))*0
df_cr['fluxnonSS_obs_sh'] = np.ones((nCRs))*0
df_cr['fluxSS_gaps_sh'] = np.ones((nCRs))*0
df_cr['fluxnonSS_gaps_sh'] = np.ones((nCRs))*0

df_cr['N_tot_icme'] = np.ones((nCRs))*0
df_cr['N_gaps_icme'] = np.ones((nCRs))*0
df_cr['flux_obs_icme'] = np.ones((nCRs))*0
df_cr['flux_gaps_icme'] =  np.ones((nCRs))*0
df_cr['fluxSS_obs_icme'] = np.ones((nCRs))*0
df_cr['fluxnonSS_obs_icme'] = np.ones((nCRs))*0
df_cr['fluxSS_gaps_icme'] = np.ones((nCRs))*0
df_cr['fluxnonSS_gaps_icme'] = np.ones((nCRs))*0
...
for CR in range(CRstart,CRstop+1):
...
#save the data
savefile = os.getenv('DBOX') + 'Papers_WIP\\_coauthor\\ManuelaTemmer\\osf_icme.csv'
